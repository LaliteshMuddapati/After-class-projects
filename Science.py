...
column_alias = pd.read_sql("""SELECT experiment_name AS activity, subject AS topic, duration_mins AS time FROM experiment""", conn)
print(column_alias)
print()


table_alias = pd.read_sql("""SELECT e.experiment_name AS activity, m.item, m.quantity_g AS grams FROM experiment AS e INNER JOIN material AS m ON e.experiment_id = m.experiment_id""", conn)
print(table_alias)
print()
